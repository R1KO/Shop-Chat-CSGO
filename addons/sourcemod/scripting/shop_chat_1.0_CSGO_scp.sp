...
public ShopAction OnItemUsed(int iClient, CategoryId category_id, const char[] category, ItemId item_id, const char[] item, bool isOn, bool elapsed)
{
	...
	if(index == PREFIX)
	{
		...
		if(strcmp(g_sClientPrefix[iClient], "custom") == 0)
		{
			...
			PrintToChat(iClient, " \x04[SHOP] \x01Для смены/установки префикса введите !tag \"Ваш префикс\"");
		}
	}
	else
	{
	...
	if(iClient) 
	{
		if(g_bClientPrefix[iClient]) 
		{
			...
			if(sBuffer[0])
			{
				...
				PrintToChat(iClient, " \x04[SHOP] \x01Вы установили себе префикс \"%s\".", sBuffer);
			}
		}
		else
		{
			PrintToChat(iClient, " \x04[SHOP] \x02Чтобы его использовать префикс приобретите его в магазине!");
		}
	}
